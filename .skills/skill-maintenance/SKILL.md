...
6. **AGENTS.md table in sync**: the skill table lists every skill with an
   accurate trigger; removed skills are gone from it. Exception (deliberate):
   the five Boost-managed skills (`echo-development`,
   `inertia-react-development`, `infer-conventions`, `laravel-best-practices`,
   `tailwindcss-development`) are intentionally excluded from the table — they
   are regenerated by `php artisan boost:update` (see `boost.json` `skills[]`)
