...
| KI-020 | CI: a11y "Room dialogs" test fails clicking `.media-button--settings` — "Element is not visible" even with `force: true` | Video.js (@videojs/react) auto-hides the control bar ~2s after the last pointer move: `.media-controls--root` drops its `data-visible` attribute and the bars fade to `opacity: 0` + `pointer-events: none`. Worse, while a media-error dialog is open, `.media-error[data-open] ~ .media-controls * { visibility: hidden }`. `force: true` cannot pierce `visibility: hidden` (Playwright still needs a visible box for scroll-into-view), and the mock video URL (`https://www.example.com/video.mp4`, proxy mode) always fails, so the error dialog opens — asynchronously on slow CI, often AFTER the test's one-shot `isVisible()` dismiss check. | Always reveal the control bar before clicking a control-bar button: `revealPlayerControls(page)` in `contrast-a11y.spec.ts` waits for/dismisses any media-error dialog, hovers the player container, and waits for `.media-controls--root[data-visible]` before a normal (non-forced) click. Never force-click control-bar buttons. | `tests/a11y/contrast-a11y.spec.ts`, `@videojs/react` skin (`media-controls--root`, `media-error[data-open]`), `@videojs/core` `ControlsDataAttrs.visible` |
