...
| KI-019 | a11y "Room dialogs" test fails on offset slider focus and reset link dark contrast | Subtitle settings redesign moved the sync offset slider to the "پیشرفته" tab and renamed the reset button to "بازنشانی هم‌زمانی", causing selector timeouts in `contrast-a11y.spec.ts`. In addition, the reset button's `text-primary` class failed WCAG AA contrast (3.04:1) against dark card backgrounds (`#111117`). | In `contrast-a11y.spec.ts`, switch to "پیشرفته" tab before locating slider and update button label. In `subtitle-settings.tsx`, replace `text-primary` with `text-accent-foreground` for proper dark contrast. | `tests/a11y/contrast-a11y.spec.ts`, `resources/js/Components/composite/subtitle-settings.tsx` |
| KI-020 | CI: a11y "Room dialogs" test can't click `.media-button--settings` — "Element is not visible" even with `force: true`; or the `.media-error` close button keeps detaching/re-appearing until the test times out | Two stacked causes. (1) The a11y room fixture (`with_video=1`) uses proxy mode (`video_url=https://www.example.com/video.mp4`), so the player fetches `/proxy/video/{room}`; under CI's single-process `php artisan serve` that proxy self-requests the same server and stalls, so Video.js raises a media-error dialog. On slow CI the dialog opens asynchronously — often AFTER a one-shot dismiss check — and once its close button is clicked the dialog keeps re-opening (the video keeps failing), so the close click never stabilizes. (2) While the error dialog is open, the skin forces `.media-error[data-open] ~ .media-controls * { visibility: hidden }` — `force: true` cannot pierce that (Playwright still needs a visible box for scroll-into-view). The control bar also auto-hides ~2s after the last pointer move (`.media-controls--root` drops `data-visible`). | Definitively fix the media, don't fight the dialog: `installPlayableVideoMock()` in `tests/a11y/room-nav.ts` intercepts `/proxy/video/\d+/` and fulfills it with `public/videos/sample.mp4` (200) so the media loads, no error dialog ever appears, and the single-process dev server is not self-deadlocked. Then `revealPlayerControls()` in `contrast-a11y.spec.ts` hovers the player and waits for `.media-controls--root[data-visible]` before a normal click; never force-click control-bar buttons. | `tests/a11y/room-nav.ts`, `tests/a11y/contrast-a11y.spec.ts`, `routes/test-helpers.php` (`with_video` proxy fixture), `@videojs/react` skin (`.media-error[data-open]`, `.media-controls--root`), `@videojs/core` `ControlsDataAttrs.visible` |
| KI-021 | Production: guests' playback lags 0–60s behind the host, freezes at the join-time state, or yanks/jumps when bursts of updates land; nothing reproduces locally or in E2E | Room broadcasts were queued `ShouldBroadcast` while production runs `QUEUE_CONNECTION=database` drained once a minute by the `queue:work` cron tick — every playback/presence event waited for the next minute before reaching Pusher. Compounded by the hooks disabling polling whenever Echo was merely *configured* (`pushEnabledRef = echo !== null`), so an unconnected socket had no fallback at all. E2E never caught it: CI runs `BROADCAST_CONNECTION=null` (polling). | (1) `PlaybackStateChanged`/`MemberPresenceChanged`/`NewChatMessage` implement `ShouldBroadcastNow` (inline `dispatchNow`, never queued; Pusher HTTP timeout capped at 3s via `PUSHER_TIMEOUT`). (2) `watchPushHealth()` in `lib/echo.ts` (connection `state_changed` + channel `pusher:subscription_*`); both hooks poll only while unhealthy and re-seed from the GET on healthy transitions. Regression tests: `*_broadcast_is_not_queued_on_the_database_queue` (PlaybackSyncTest/PresenceTest/ChatTest) + hook tests for the health-gated fallback. | `app/Events/*.php`, `config/broadcasting.php`, `resources/js/lib/echo.ts`, `Hooks/use-playback-sync.ts`, `Hooks/use-presence.ts` |
| KI-022 | Production: chat shows spurious "X از اتاق خارج شد" + "X به اتاق پیوست" pairs though nobody left (typically after a member's mobile network blip or backgrounded tab) | `usePresence` mapped Pusher's connection-level `leaving` (`member_removed` — fired on ANY socket drop) onto a roster removal, which the polling-era moment diff rendered as a real leave; the reconnect's `joining` then produced the re-join. The 90s presence-timeout grace never applied to this path. | `leaving` is ignored entirely; `here`/`joining` only seed the member list optimistically (`seedRoster`, no baseline change); join/leave moments derive exclusively from authoritative server rosters (`.member.presence.changed` broadcasts + GETs). Real leaves/kicks surface via the roster (near-realtime since KI-021's sync broadcasts) or the 90s timeout. | `Hooks/use-presence.ts`, `lib/presence-moments.ts` |
