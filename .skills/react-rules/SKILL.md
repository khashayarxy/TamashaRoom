...
The generic Boost skill `inertia-react-development` is regenerated by
`php artisan boost:update` and must not be hand-edited. Where it conflicts
with this project, this skill wins:

- **This project does NOT use Inertia's `usePoll`.** Live room data (playback
  state, presence, chat) uses the custom `usePlaybackSync` / `usePresence`
  axios-based polling hooks instead (see `laravel-backend-rules` for the
  tiered-cadence pattern). **Do not use `usePoll` in this codebase.**
