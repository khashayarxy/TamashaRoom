...
## Boost Overrides (read first)

The generic Boost skills `laravel-best-practices` and `echo-development` are
regenerated by `php artisan boost:update` and must not be hand-edited. Where
they conflict with this project, this skill wins:

- **This project's shared-hosting constraints forbid Redis, `Cache::tags`,
  Horizon, and Laravel Reverb** — ignore any Boost skill guidance suggesting
  these. Cache/queue/session are database-backed, broadcasting is
  Pusher/Apinator (never a self-hosted Reverb server on this hosting), and
  there is no persistent worker (see "The Hosting Constraint" below).
