...
|---|---|---|---|
| **Landing (Welcome)** | Persuade | Near-black indigo-tinted hero with Vazirmatn display weight and a single CTA. Signature: a **live demo frame** — a realistic video-player mockup anchored on the visual-end side (dark card, 1px indigo border, solid indigo play button, **rose** seekbar fill, Persian-digit timecode, green presence dot + "X نفر آنلاین"), with a floating rose reaction bubble (friend avatar + short reaction line) hanging off its bottom-outer edge. Text column on the visual-start side: rose eyebrow pill with a live-status dot, a short (3–5 word) headline with a thin indigo rule beneath, one subhead line. **Color directive:** indigo = primary actions/focal points (CTA, play button, rule), rose = liveness/social signals (eyebrow, seekbar fill, reaction bubble), green = online-status dots only. No gradients, no glassmorphism, no feature grid. The mock seekbar fills from the right (RTL-native) — the mockup is decorative Persuade-mode illustration, not the real Operate-mode player (which stays LTR). |
