...
interface RoomSettingsProps {
    ...
    onUpdate: (data: { name?: string; invite_code?: string; is_locked?: boolean }) => void;
}

export function RoomSettingsDialog({ open, onClose, room, onUpdate }: RoomSettingsProps) {
    ...
    const handleRegenerate = async () => {
        ...
        try {
            const { data } = await api.post(`/rooms/${room.id}/regenerate-invite`);
        ...
        <Dialog open={open} onClose={onClose}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>مدیریت تنظیمات و دسترسی اتاق</DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button type="button" variant="outline" size="sm" onClick={handleRegenerate} disabled={regenerating} title="ساخت کد جدید">
                                {regenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Key className="h-4 w-4" />}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between rounded-xl border border-input p-3">
                        <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                                {isLocked ? 'اعضای جدید نمی‌توانند بپیوندند' : 'هر کسی با کد دعوت می‌تواند بپیوندد'}
                            </div>
                        </div>
                        <Button
                            type="button"
