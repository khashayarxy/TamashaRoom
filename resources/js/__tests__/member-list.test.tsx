...
describe('MemberList', () => {
    ...
    it('renders member names', () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} connected />);
    ...
    it('shows online count', () => {
        const members = [
            makeMember({ id: 1, user_id: 1, name: 'Alice', presence_status: 'online' }),
            makeMember({ id: 2, user_id: 2, name: 'Bob', presence_status: 'offline' }),
        ];
        render(<MemberList members={members} roomId={1} ownerId={1} connected />);
    ...
    it('shows disconnected state', () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} connected={false} />);
    ...
    it('shows kick and transfer buttons for owner on non-owner members', () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} currentUserId={1} connected />);
    ...
    it('does not show action buttons for non-owner viewer', () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} currentUserId={2} connected />);
    ...
    it('shows room settings button for owner', () => {
        const members = [makeMember({ id: 1, user_id: 1, name: 'Owner', is_owner: true })];
        render(<MemberList members={members} roomId={1} ownerId={1} currentUserId={1} connected />);
    ...
    it('calls onKick after kick confirmation', async () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} currentUserId={1} onKick={onKick} connected />);

        await user.click(screen.getByTitle('حذف از اتاق'));

        expect(screen.getByRole('heading', { name: /حذف Target/ })).toBeInTheDocument();
    ...
    it('calls onTransfer after transfer confirmation', async () => {
        ...
        render(<MemberList members={members} roomId={1} ownerId={1} currentUserId={1} onTransfer={onTransfer} connected />);

        await user.click(screen.getByTitle('انتقال مالکیت'));

        expect(screen.getByRole('heading', { name: /انتقال مالکیت/ })).toBeInTheDocument();

        const confirmButtons = screen.getAllByRole('button', { name: 'انتقال مالکیت' });
    ...
    it('shows last seen for offline members', () => {
        const members = [
            makeMember({ id: 1, user_id: 1, name: 'OfflineUser', presence_status: 'offline', last_seen_at: new Date(Date.now() - 3600000).toISOString() }),
        ];
        render(<MemberList members={members} roomId={1} ownerId={1} connected />);
    ...
    it('renders status indicator colors', () => {
        const members = [
            makeMember({ id: 1, user_id: 1, name: 'Online', presence_status: 'online' }),
            makeMember({ id: 2, user_id: 2, name: 'Away', presence_status: 'away' }),
            makeMember({ id: 3, user_id: 3, name: 'Offline', presence_status: 'offline' }),
        ];
        const { container } = render(<MemberList members={members} roomId={1} ownerId={1} connected />);
