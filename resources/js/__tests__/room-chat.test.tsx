...
function makeMessage(overrides: Partial<{ id: number; user_id: number; body: string; created_at: string; user: { id: number; name: string } }> = {}) {
    return {
        id: 1,
        user_id: 1,
        body: 'Test message',
...
describe('RoomChat', () => {
    ...
    it('renders initial messages', () => {
        const messages = [makeMessage({ id: 1, body: 'Hello' }), makeMessage({ id: 2, body: 'World', user_id: 2 })];
    ...
    it('shows delete button only on own messages', () => {
        ...
        const deleteButtons = screen.getAllByRole('button', { name: 'حذف پیام' });
        expect(deleteButtons).toHaveLength(1);
    });

    it('opens confirm dialog when delete button is clicked', async () => {
        const user = userEvent.setup();
        const messages = [makeMessage({ id: 1, user_id: 1, body: 'To delete' })];
        render(<RoomChat roomId={1} initialMessages={messages} />);

        const deleteBtn = screen.getByRole('button', { name: 'حذف پیام' });
        await user.click(deleteBtn);

        expect(screen.getByText('آیا از حذف این پیام اطمینان دارید؟ این عمل قابل بازگشت نیست.')).toBeInTheDocument();
    ...
    it('calls api.delete and removes message on confirm', async () => {
        ...
        const messages = [makeMessage({ id: 42, user_id: 1, body: 'Delete me' })];
        render(<RoomChat roomId={1} initialMessages={messages} />);
    ...
    it('sends a message on form submit', async () => {
        const user = userEvent.setup();
        mockPost.mockResolvedValue({ data: makeMessage({ id: 3, body: 'New message' }) });
        ...
        await waitFor(() => {
            expect(mockPost).toHaveBeenCalledWith('/chat/1/messages', { body: 'New message' });
        });
    ...
    it('polls for new messages on interval', async () => {
        mockGet.mockResolvedValue({ data: [makeMessage({ id: 1, body: 'New polled message' })] });
