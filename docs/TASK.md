...
## Testing
- [x] **220** PHPUnit tests across all modules (**179 Feature + 41 Unit**) — verified by runtime run on 2026-08-02: 220 passed by `php artisan test` (749 assertions). No data providers — static count equals runtime count. (This is the **canonical count**; skills reference `docs/TASK.md` rather than hardcoding it. Incremented from 216 by 4 new tests added in the Quick Wins Phase 8 batch.)
- [x] **131** Frontend Vitest tests (**108 component/hook/logic + 23 Zustand store tests**: theme 5, room-ui 11, subtitle 7) — verified by runtime run on 2026-08-02: 131 passed by `npm run test`. No parameterized (`it.each`/`test.each`) tests — static count equals runtime count. (Incremented from 124 by 7 new tests added in the Frontend UX Fixes batch.)
