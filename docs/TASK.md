...
- [x] **190** Frontend Vitest tests (**165 component/hook/logic + 25 Zustand store tests**: theme 5, room-ui 11, subtitle 9) — verified by runtime run on 2026-08-03: 190 passed by `npm run test`. No parameterized (`it.each`/`test.each`) tests — static count equals runtime count. (Incremented from 156 by 34 new tests added in the Phase 14 session-moments batch.)
- [x] **14** Playwright E2E tests passing (chat 2, lock-kick-transfer 4, room 3, subtitle 4, presence-moments 1) — verified 2026-08-03 with `APP_ENV=local`. (Incremented from 13 by 1 new test added in the Phase 14 session-moments batch.)
...
- [x] **Email-verification gating audited, NOT weakened** — the `verified` middleware is the primary anti-abuse deterrent (disposable-account room spam; register is per-IP throttled but IP throttling is evadable by botnets). Removing `verified` from room routes would let unbounded unverified accounts create rooms (the 50-room cap) and join to spam chat. No alternative was proven safe by the auth/security audit, so the gate stays; the onboarding fix is carry-through, not de-gating.
